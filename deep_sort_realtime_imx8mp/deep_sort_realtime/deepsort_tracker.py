...
class DeepSort(object):
    def __init__(
        ...
        if embedder is not None:
            ...
            elif embedder == "mobilenet_imx_tflite_npu":
                from deep_sort_realtime.embedder.embedder_tflite_npu import (
                    MobileNetTFLiteNPU_Embedder as Embedder,
                )
                
                # embedder_wts should be the path to your TFLite model
                if not embedder_wts:
                    raise ValueError("embedder_wts (path to TFLite model) is required for tflite_npu embedder")

                if not embedder_model_name:
                    raise ValueError(" ERROR: embedder_model_name has been re-used as a path of VX_Delegate and it is required tflite NPU execution")

                vx_delegate_path = embedder_model_name
                
                self.embedder = Embedder(
                    model_path=embedder_wts,
                    vx_delegate_path=vx_delegate_path,
                    use_npu=not embedder_gpu,  # Use NPU when GPU is disabled
                    bgr=bgr,
                    max_batch_size=16,
                )
